...
const SigningDetails = () => {
  ...
  useEffect(() => {
    ...
    .then((response) => {
        console.log(response.data.will.signingDetails);
        setInputFields(response.data.will.signingDetails);
    })
    .catch((error) => {
        console.log(error);
    });
      ...
      <Form className={classes.root} onSubmit={handleSubmit}>
        {inputFields.map((inputField) => (
          ...
          <Button
            ...
            color="primary"
            onClick={() => {
              window.location.href = "/managewill/additional?will_id=" + parseURLParams(window.location.href).will_id[0];
            }}
          >
          Back
        </Button>
