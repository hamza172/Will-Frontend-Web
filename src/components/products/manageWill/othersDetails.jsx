...
const OtherDetails = ({ history }) => {
  ...
  const submitHandler = (e) => {
    ...
    if (form.checkValidity() === false) {
      ...
      window.location.href = "/managewill/pet?will_id=" + parseURLParams(window.location.href).will_id[0];
    }
  ...
  useEffect(() => {
    ...
    .then((response) => {
        console.log(response.data.will.otherDetails);
        setBeneficiary(response.data.will.otherDetails.transferBeneficiary);
        if(response.data.will.otherDetails.trusteeAdd) {
            setShowFields(true);
            setTrusteeName(response.data.will.otherDetails.trusteeName);
            setTrusteeAdd(response.data.will.otherDetails.trusteeAdd);
        }
        else {
            setShowFields(false);
        }
        setGift(response.data.will.otherDetails.giftMadeTo);
        setName(response.data.will.otherDetails.name);
        setRelationship(response.data.will.otherDetails.relationship);
        setAddress(response.data.will.otherDetails.address);
        setContest(response.data.will.otherDetails.contest);
    })
    .catch((error) => {
        console.log(error);
    });
